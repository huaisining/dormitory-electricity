...
if TURSO_URL:
    ...
    import sqlite3
    DB_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "backend", "dorm.db")
    def get_db():
...
def init_db():
    ...
    db.executescript("""
        CREATE TABLE IF NOT EXISTS rooms (
            ...
            campus TEXT NOT NULL, building_code TEXT NOT NULL, building_name TEXT NOT NULL,
            floor TEXT NOT NULL, room_code TEXT NOT NULL, room_name TEXT NOT NULL,
        ...
        CREATE TABLE IF NOT EXISTS records (
            ...
            payer_student_id TEXT NOT NULL DEFAULT '', payer_name TEXT NOT NULL,
            kwh REAL DEFAULT 0, settled INTEGER DEFAULT 0,
            source TEXT DEFAULT 'manual', elec_type TEXT DEFAULT '',
            created_at TEXT DEFAULT (datetime('now','localtime'))
        );
...
# Add lighting columns (safe migration)
try:
    _db = get_db()
...
def serve_index():
    ...
    if not os.path.exists(idx_path):
        idx_path = os.path.join(base, "..", "backend", "index.html")
    if not os.path.exists(idx_path):
        idx_path = os.path.join(base, "index.html")
...
def serve_manifest():
    ...
    if not os.path.exists(mp):
        mp = os.path.join(base, "..", "backend", "manifest.json")
    if not os.path.exists(mp):
        mp = os.path.join(base, "..", "manifest.json")
...
def serve_sw():
    base = os.path.dirname(os.path.abspath(__file__))
    sp = os.path.join(base, "sw.js")
    if not os.path.exists(sp):
        sp = os.path.join(base, "..", "backend", "sw.js")
    if not os.path.exists(sp):
        sp = os.path.join(base, "..", "sw.js")
...
def serve_icon(size):
    base = os.path.dirname(os.path.abspath(__file__))
    ip = os.path.join(base, f"icon-{size}.png")
    if not os.path.exists(ip):
        ip = os.path.join(base, "..", "backend", f"icon-{size}.png")
    if not os.path.exists(ip):
        ip = os.path.join(base, "..", f"icon-{size}.png")
